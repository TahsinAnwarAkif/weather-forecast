package com.oddle.app.weather.controller.helper;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.oddle.app.weather.controller.exception.RestTemplateExceptionResolver;
...
import static com.oddle.app.weather.util.ReflectionUtils.getFieldValue;
import static com.oddle.app.weather.util.ReflectionUtils.getJsonIgnoredFieldList;
import static com.oddle.app.weather.util.StringUtils.nullSafeString;
...
public class CityWeatherDataHelper {
    ...
    public CityWeatherData saveCurrentCityWeatherData(String city) throws JsonProcessingException {
        CityWeatherData cwd = getObjectFromJson(getCurrentCityWeatherDataJson(city), CityWeatherData.class);
    ...
    private void prepareCityWeatherDataJsonIgnoredFields(CityWeatherData cwd, long id) {
        ...
        getJsonIgnoredFieldList(cwd).forEach(field -> {
            field.setAccessible(true);

            try {
                field.set(cwd, getFieldValue(cwdBeforeUpdate, field.getName()));
    ...
    private String getWeatherApiUrlWithCity(String city) {
        return CWD_API_URL.replace("{API key}", CWD_API_KEY)
                .replace("{city name}", nullSafeString(city));
    }
}
